...
6. **Mining history** lists the current principal's recent and running batches retained for 90 days. Select any batch to inspect it through the same full directory, coverage ledger, intermediate, questionnaire, and knowledge-cloud interface. Switching history does not stop background polling or automatic incremental stages. Follow separate document and Memory `task_id`, `status`, and `stage` values; the current running Compile can be cancelled cooperatively, and failed or cancelled tasks can be resumed from a checkpoint without uploading files again.
