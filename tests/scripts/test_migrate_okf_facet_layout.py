...
def test_migration_moves_facets_to_root_and_rewrites_references(tmp_path: Path):
    ...
    assert [move.new_relative for move in moves] == [
        "knowledge/what/search",
        "knowledge/why/search",
        "knowledge/how/search",
    ]
    ...
    assert (wiki / "knowledge/what/search/search.md").read_text() == "what"
    assert (wiki / "knowledge/why/search/search-rationale.md").read_text() == "why"
    assert (wiki / "knowledge/how/search/search-operation.md").read_text() == "how"
    assert "knowledge/what/search/search.md" in (wiki / "index.md").read_text()
    ledger = (wiki / "_mining/evidence-ledger.json").read_text()
    assert "knowledge/why/search/search-rationale.md" in ledger
    assert "wiki/knowledge/how/search/search-operation.md" in ledger
...
def test_plan_refuses_to_merge_an_existing_target(tmp_path: Path):
    wiki = _legacy_wiki(tmp_path)
    target = wiki / "knowledge/what/search"
